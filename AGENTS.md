...
1.雨课堂：
- 雨课堂-成绩单.xlsx：包含5个sheet：
    
| sheet | 内容 | 说明 |
| ----- | ---- | ---- |
| 1     | 班级成绩单 | 包含班级所有学生的成绩，分5个方面计算总成绩：讨论(10.0%)	作业(30.0%)	考试(30.0%)	视频(20.0%)	图文(10.0%) |
| 2     | 视频学习单元得分明细表 | 包含每个视频学习单元的完成率，前四列是学生基本信息，后面的列为已发布的视频学习单元的得分比例（汇总后与sheet1的视频列相乘得出最终得分）。列名格式为：视频学习单元名称（例如：第1章）-时间戳（例如：2026-03-12 08:13:13）总分（例如：（满分100.0分）） |
| 3     | 图文学习单元得分明细表 | 与sheet 2的计算规则类似 |
| 4     | 已发布的讨论得分明细表 | 与sheet 2的计算规则类似 |
| 5     | 作业得分明细表 | 与sheet 2的计算规则类似 |

- 雨课堂-数据统计.xlsx：包含2个sheet，第1个sheet是班级的学习情况统计。第2个sheet是每个学生的的学习过程数据统计，主要包含：课堂数据统计、课件数据统计、视频（课件插入的慕课视频+单个发布视频）	、习题（课堂习题+课件习题+作业习题）、发布的讨论、发布的考试。

- 雨课堂-知识图谱-学生汇总.xls：包含1个sheet，主要是学生对雨课堂的知识图谱功能模块中发布的知识点的汇总学习情况统计，主要包括：知识点掌握率、知识点学习内容完成个数（未开始/进行中/已完成）、完成率、自测题作答情况（未作答/已作答）、正确情况（正确/错误）、正确率。

- 雨课堂-知识图谱-学生实时明细.xls：包含1个sheet，主要是学生对雨课堂的知识图谱功能模块中发布的知识点的明细学习情况统计，主要包含：完成率、自测习题正确率、掌握率。

- 雨课堂-知识图谱-知识点汇总.xls：包含1个sheet，主要包含了针对每个知识点发布的学习内容及其学习情况统计，包含：完成情况（未开始/进行中/已完成）、完成率、自测习题数量、作答情况（未作答/已作答）、完成率、正确率。

2.头歌：

- 头歌_总成绩.xls：包含1个sheet，主要是头歌的总成绩统计，前7列为学生基本信息，第8列为总成绩，后面的列为每个课堂实验的成绩（例如：*鸡兔同笼求解进阶编程实现），倒数第4列至倒数第1列为其他作业成绩。

- 头歌_活跃度.xls：包含1个sheet，主要是学生在头歌平台的活跃度统计，计算规则如下：
    活跃度 = 作业完成数（*10）+试卷完成数（*10）+问卷完成数（*7）+资源发布数（*5）+帖子发布数（*2）+帖子回复数（*1）+作业回复数（*1）

- 头歌_作业成绩表.xls：包含N个sheet，主要是对头歌的每项课堂实验成绩进行分别统计，每个sheet为该课堂实验的全班成绩明细，包含学生基本信息、课堂实验成绩，每个实验可能有多个关卡，表中有多个关卡的完成情况详情。
